...
public class ReflectKit {
	 ...
	 *
	 * @param clazz     指定的类
	 * @param name      要获取的字段的名字
	 * @param declared  是否查找Declared字段
	 * @param recursion 是否向上去其父类中寻找
	 * @return
	 */
	public static Field getField(Class<?> clazz, String name, boolean declared, boolean recursion) {
		Field field = null;
		try {
			field = declared ? clazz.getDeclaredField(name) : clazz.getField(name);
		} catch (NoSuchFieldException e) {
			clazz = clazz.getSuperclass();
			if (recursion && clazz != null) {
				return getField(clazz, name, declared, recursion);
			}
		}
		return field;
	}
	...
	public static Field getField(Class<?> clazz, String field) {
		return getField(clazz, field, true, true);
	}
	 ...
	 *
	 * @param clazz               给定的类
	 * @param declared            是否需要获取Declared字段
	 ...
	 * @return 给定类的所有字段
	 */
	public static List<Field> getFields(Class<?> clazz, boolean declared, boolean isGetParentField, boolean isGetAllParentField, boolean isDESCGet) {
		...
		if (isGetParentField) {
			...
			if (isGetAllParentField) {
				classList = getSuperClasss(clazz, true);
			} else {
			...
			if (isDESCGet) {
				for (int w = classList.size() - 1; w > -1; w--) {
					for (Field field : declared ? classList.get(w).getDeclaredFields() : classList.get(w).getFields()) {
						list.add(field);
					}
				}
			} else {
	...
	public static <T> T getObjectByFieldName(Object object, String fieldName, Class<T> clas) {
		if (object != null && ValidateKit.notEmpty(fieldName) && clas != null) {
			try {
